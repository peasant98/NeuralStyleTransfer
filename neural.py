...
class NeuralStyle():
    ...
    def create_white_noise(self):
        self.input_img = self.content_img.clone().to(device)

        # white noise!!
        B, C, H, W = self.content_img.size()
        self.input_img = torch.randn(B, C, H, W).mul(0.001).to(self.device)

    def get_input_optimizer(self, input_img, name='lbfgs'):
    ...
    def run_style_transfer(self, num_steps=1000,
                           style_weight=500, content_weight=5):
        """
        Run the style transfer.
        """
        print('Building the style transfer model..')
        self.create_white_noise()
        model, style_losses, content_losses, tv_losses = self.get_style_model_and_losses(model_dict,
        ...
        def closure():
            ...
            if run[0] % 100 == 0:
                ...
                i = run[0]
                deprocess(self.input_img).save(f'generated/out_{i}.png')

            return loss

        optimizer, loop_val = get_input_optimizer(self.input_img)
        print(optimizer)
...
def set_style_scale(scale):
    style_scale = scale


def get_img(main_folder_path, img_name=None):
    style_scale = 1.0
    size = 314
    list_img = os.listdir(main_folder_path)
    if img_name is None:
        full_img = np.random.choice(list_img)
    else:
        ...
        full_img = img_name
    full_img_string = os.path.join(main_folder_path, full_img)
    style_size = size * style_scale


style_scale = 1.0

list_img = os.listdir('content')
# img_string = np.random.choice(list_img)
img_string = 'mother2.jpg'
full_img_string = os.path.join('content', img_string)
size = 314
style_size = size * style_scale
content_img = preprocess(full_img_string, size).to(device)
list_img = os.listdir('style')
style_string = np.random.choice(list_img)

# style one
full_style_string = os.path.join('style', "scream.jpg")

style_img = preprocess(full_style_string, size).to(device)


plt.figure()

plt.imshow(deprocess(style_img))
plt.show()
plt.figure()
plt.imshow(deprocess(content_img))
plt.show()
...
# layers to get style and content loss...
content_layers_default = ['relu4_2']
style_layers_default = ['relu1_1', 'relu2_1', 'relu3_1', 'relu4_1', 'relu5_1']


def get_style_model_and_losses(cnn, model_dict, style_img, content_img,
    ...
    for layer in cnn.children():
        if isinstance(layer, nn.Conv2d):
            ...
            layer_name = layer.__class__.__name__
            raise RuntimeError("Unrecognized Layer: {layer_name}")
        model.add_module(name, layer)
...
input_img = torch.randn(B, C, H, W).mul(0.001).to(device)


def get_input_optimizer(input_img, name='lbfgs'):
    # this line to show that input is a parameter that requires a gradient
    if name == 'lbfgs':
        optim_state = {
                'max_iter': 1000,
        ...
        loop_val = 1
        optimizer = optim.LBFGS([input_img], **optim_state)
    else:
        print("Running optimization with ADAM")
        optimizer = optim.Adam([input_img], lr=0.01)
        loop_val = 1

    return optimizer, loop_val
...
def run_style_transfer(cnn,
    ...
    """
    print('Building the style transfer model..')
    model, style_losses, content_losses, tv_losses = get_style_model_and_losses(cnn, model_dict,
                        style_img, content_img)
    ...
    def closure():
        ...
        if run[0] % 100 == 0:
            ...
            os.makedirs('generated', exist_ok=True)
            i = run[0]
            deprocess(input_img).save(f'generated/out_{i}.png')

        return loss

    optimizer, loop_val = get_input_optimizer(input_img)
    print(optimizer)
    while run[0] <= loop_val:
        optimizer.step(closure)
        # a last correction...
...
if __name__ == '__main__':
    ...
    neural_style_system.run_style_transfer(style_weight=250, content_weight=5)
